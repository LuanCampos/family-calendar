...
// SEC-011: Changed from 'budget-offline-db' to 'family-calendar-offline-db'
const DB_NAME = 'family-calendar-offline-db';
const DB_VERSION = 6;
...
const openDB = (): Promise<IDBDatabase> => {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // Tag definitions store
      ensureStore('tag_definitions', { keyPath: 'id' }, (store) => {
        if (!store.indexNames.contains('family_id')) {
          store.createIndex('family_id', 'family_id', { unique: false });
        }
      });

      // Event tags junction store
      ensureStore('event_tags', { keyPath: 'id' }, (store) => {
        if (!store.indexNames.contains('event_id')) {
          store.createIndex('event_id', 'event_id', { unique: false });
        }
        if (!store.indexNames.contains('tag_id')) {
...
export const isOfflineId = (id: string | undefined): boolean => {
  if (!id) return false;
